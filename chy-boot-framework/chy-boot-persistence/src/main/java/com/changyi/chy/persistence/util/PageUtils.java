...
public class PageUtils {
    ...
    /**
     * 将MyBatis-Plus的IPage对象转换为统一返回的R对象
     *
     * @param page MyBatis-Plus的分页结果
     * @param <T>  实体类型
     * @return 统一返回的R对象
     */
    public static <T> R<PageResult<T>> toResult(IPage<T> page) {
        return R.data(convertToPageResult(page));
    }
    ...
    /**
     * 将实体分页结果转换为DTO分页结果，并封装为统一返回的R对象
     *
     * @param page    实体分页结果
     * @param mapper  实体到DTO的转换函数
     * @param <T>     实体类型
     ...
     * @return 统一返回的R对象
     */
    public static <T, R> R<PageResult<R>> toResultDto(IPage<T> page, Function<T, R> mapper) {
        return R.data(convertToDto(page, mapper));
    }
     ...
     * @return 统一返回的R对象
     */
    public static <T> R<PageResult<T>> manualPagingResult(List<T> list, long current, long size) {
        return R.data(manualPaging(list, current, size));
    }
} 